...
def train_command(
    ...
    resize_size: Tuple[int, int] = typer.Option((0, 0), help="Override the pre-crop resize, e.g. 46 46 to resize directly to the input size with no crop augmentation (0 0 = preset)."),
    input_size: Tuple[int, int] = typer.Option((0, 0), help="Override the network input size, e.g. 64 64 (0 0 = preset). Combine with --resize-size 64 64 for a no-crop 64x64 scenario."),
    neighbor_label_jitter: float = typer.Option(0.0, min=0.0, help="Uniform +-J deg online jitter on neighbour-frame labels (they copy their anchor's integer angle; ~1.5 spreads the 1-deg spikes)."),
    ...
    result = train_model(
        ...
        resize_size=None if resize_size == (0, 0) else resize_size,
        input_size=None if input_size == (0, 0) else input_size,
        neighbor_label_jitter_deg=neighbor_label_jitter if neighbor_label_jitter > 0 else None,
...
def train_subcommand(
    ...
    resize_size: Tuple[int, int] = typer.Option((0, 0)),
    input_size: Tuple[int, int] = typer.Option((0, 0)),
    neighbor_label_jitter: float = typer.Option(0.0, min=0.0),
